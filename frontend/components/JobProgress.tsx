...
export function JobProgress({ jobId, onDone, onFailed, showOutput = false }: Props) {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
          ...
          return (
            <div key={step.key} className="flex items-center gap-2">
              ...
              {i < steps.length - 1 && (
                <div
                  className="w-8 h-px"
                  style={{ background: past ? 'var(--accent)' : 'var(--border)' }}
                />
              )}
            </div>
          )
      ...
      {showOutput && output && (
        <div
          className="rounded-xl p-4 text-xs font-mono leading-relaxed whitespace-pre-wrap max-h-64 overflow-y-auto"
          style={{ background: '#080f1a', color: '#a3e635' }}
        >
          {output}
          {status === 'RUNNING' && (
